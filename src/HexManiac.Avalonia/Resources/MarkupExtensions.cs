...
namespace HavenSoft.HexManiac.Avalonia.Resources {
   ...
   public class TextGeometryExtension : MarkupExtension {
      ...
      public override object ProvideValue(IServiceProvider serviceProvider) {
         var formattedText = new FormattedText(Text, CultureInfo.CurrentCulture, FlowDirection.LeftToRight, new Typeface("Calibri"), Size, Brushes.Black);
   ...
   //      return icons.Find<Geometry>(name);
   //   }
   //}
   ...
   public class MethodCommandExtension : MarkupExtension {
      ...
      public override object ProvideValue(IServiceProvider serviceProvider) {
         ...
         if (valueProvider.TargetObject.GetType().FullName == "Avalonia.SharedDp") {
            return this; // we're in a template: defer execution
         }
         var element = (Control)valueProvider.TargetObject;
   ...
   public class CommandExtension : MarkupExtension {
      ...
      public override object ProvideValue(IServiceProvider serviceProvider) {
         if (!commands.TryGetValue(Command, out var command)) {
            command = new RoutedCommand(Command);
